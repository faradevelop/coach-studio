...
/// `email`/`token` are pre-filled when the user follows the reset link
/// from the notification email (`?email=...&token=...`, see
/// `AppServiceProvider::boot()`'s `ResetPassword::createUrlUsing`), but
    ...
    try {
      ...
      if (success) {
        sl<AppNotification>().success(
          'رمز عبور با موفقیت تغییر یافت. اکنون وارد شوید.',
        );
        context.goNamed(AppRouteNames.login);
      } else {
        sl<AppNotification>().error('لینک بازیابی نامعتبر یا منقضی شده است.');
      }
    } finally {
